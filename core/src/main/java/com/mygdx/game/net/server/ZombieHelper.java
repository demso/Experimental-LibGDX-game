...
public class ZombieHelper {
    ...
    public ZombieMove getMove(ZombieInfo zinfo) {
        ...
        return new ZombieMove().set(zinfo.id, getPlayerTarget(zinfo).name, zinfo.x, zinfo.y, zinfo.xSpeed, zinfo.ySpeed);
    }


    public void fillMove(ZombieInfo zinfo, ZombieMove move) {
        move.set(zinfo.id, zinfo.target, zinfo.x, zinfo.y, zinfo.xSpeed, zinfo.ySpeed);
    }
}
