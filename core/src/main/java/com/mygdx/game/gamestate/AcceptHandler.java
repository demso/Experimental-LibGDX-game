...
public class AcceptHandler {
    ...
    public AcceptHandler(GameClient cli){
        client = cli;
    }
    ...
    public void update(float delta){
        ...
        if (entitiesToSpawn.size != 0) {
            for (EntityInfo inf :  new Array.ArrayIterator<>(entitiesToSpawn)){
                Entity entity = GameState.instance.entities.put(inf.id, GameState.instance.mobsFactory.spawnEntity(inf.id, inf.type, inf.x, inf.y));
        ...
        if (playersToKill.size != 0) {
            for (Player p :  new Array.ArrayIterator<>(playersToKill)){
                p.destroy();
                GameState.instance.players.remove(p.getId());
            }
        }

        if (entitiesNeedUpdate) {
            for (PlayerMove move : moves.pmoves) {
                if (GameState.instance.players.get(move.playerId) == null)
                    continue;
                GameState.instance.players.get(move.playerId).playerHandler.receivePlayerUpdate(move);
            }
    ...
    public void playerJoined(PlayerInfo plInf){
        ...
        GameState.instance.players.put(plInf.id, anotherPlayer);
    }

    public void playerExited(long playerId) {
        playersToKill.add(GameState.instance.players.get(playerId));
    }

    public void gunFire(GunFire msg){
        GameState.instance.players.get(msg.playerID).fire();
    }
