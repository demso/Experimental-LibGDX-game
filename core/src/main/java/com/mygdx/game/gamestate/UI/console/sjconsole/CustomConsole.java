...
//changed autocompletion (KeyListener TAB)
//changed refresh()
...
public class CustomConsole extends AbstractConsole {
    ...
    private CustomCommandCompleter commandCompleter;
     ...
     * @see Console#dispose()
     */
    public CustomConsole(Skin skin) {
        this(skin, true);
    }
     ...
     * @see Console#dispose()
     */
    public CustomConsole(boolean useMultiplexer) {
     ...
     * @see Console#dispose()
     */
    public CustomConsole(Skin skin, boolean useMultiplexer) {
     ...
     * @see Console#dispose()
     */
    public CustomConsole(Skin skin, boolean useMultiplexer, int keyID) {
        this(skin, useMultiplexer, keyID, Window.class, Table.class, "default-pane", TextField.class, TextButton.class,
                Label.class, ScrollPane.class);
    }

    public CustomConsole(Skin skin, boolean useMultiplexer, int keyID, Class<? extends Window> windowClass,
        ...
        commandCompleter = new CustomCommandCompleter();
        logToSystem = false;
        ...
        //consoleWindow.getStyle().background = skin.getDrawable("default-pane");
    ...
    @Override public void refresh (boolean retain) {
        ...
        this.setPosition(Math.round(stage.getWidth() - consoleWindow.getWidth()), Math.round(stage.getHeight()));
        if (retain) {
    ...
    private class ConsoleDisplay {
        ...
        ConsoleDisplay (Skin skin) {
            ...
            //scroll.getStyle().background = skin.getDrawable("default-pane");
    ...
    private class FieldListener implements TextField.TextFieldListener {
        @Override public void keyTyped (TextField textField, char c) {
            if (("" + c).equalsIgnoreCase(Input.Keys.toString(keyID))) {
    ...
    private class KeyListener extends InputListener {
        ...
        @Override public boolean keyDown (InputEvent event, int keycode) {
            ...
            if (keycode == Input.Keys.ENTER && !hidden) {
                ...
                String[] splitedInput = input.getText().split(" ");

                if (splitedInput.length == 1){
                    if (commandCompleter.isNew()) {
                        commandCompleter.set(exec, s, true);
                    }
                    ...
                    for (int i = 0; i < splitedInput.length; i ++){
                        String ss = splitedInput[i];
                        endIndex = index + ss.length();
                        if (index + ss.length() < input.getCursorPosition()) {
                            index += ss.length() + 1;
                        }
                        else {
                            entryIndex = i;
                    ...
                    if (commandCompleter.isNew()) {
                        if (entryIndex == 0 || splitedInput[0].equals("help"))
                            commandCompleter.set(exec, splitedInput[entryIndex], true);
                        else
                            commandCompleter.set(exec, splitedInput[entryIndex], false);
                    }

                    String firstHalf = input.getText().substring(0, index);
                    String completer = commandCompleter.next();
                    if (endIndex == input.getText().length())
                        input.setText(firstHalf + completer);
                    else
                        input.setText(firstHalf + completer + input.getText().substring(input.getCursorPosition()));
                    input.setCursorPosition(firstHalf.length() + completer.length());
                }
    ...
    private class DisplayListener extends InputListener {
        @Override public boolean keyDown (InputEvent event, int keycode) {
            if (disabled)
                return false;
            if (keycode == keyID && Gdx.input.isKeyPressed(Input.Keys.CONTROL_LEFT)) {
